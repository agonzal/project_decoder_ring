...
function caesar(input, shift, encode = true) {
  ...
  for (let word in words) {
    ...
    for (let letter in currentWord) {
      // iterate each word to retrieve each character.

      const current0 = currentWord[letter];

      if (current0.match(/([a-z])/g)) {
        // could also match indexOf -1.
        ...
        emptyStr += current0; //  if it isnt an alphabet character, add it to emptyStr.
      }
    }
